...
open class Visilabs {
    ...
    private init(organizationID: String, siteID: String, loggerURL: String, dataSource: String, realTimeURL: String, channel: String, requestTimeoutInSeconds: Int, restURL: String?, encryptedDataSource: String?, targetURL: String?, actionURL: String?, geofenceURL: String?, geofenceEnabled: Bool, maxGeofenceCount: Int) {
        ...
        sendQueue = [String]()
        isOnline = true // TODO: burada true'ya mı eşitlemek lazım
    ...
    func urlizeProps(_ props: [String: String]) -> String {
        ...
        for propKey in props.keys {
            ...
            if props[propKey] == nil || propKey.count == 0 {
                print("Visilabs: WARNING - property value cannot be nil or empty. Dropping property.")
                continue
            } else {
                stringValue = props[propKey]
            }

            if stringValue == nil || stringValue?.count == 0 {
                print("Visilabs: WARNING - property value cannot be nil or empty. Dropping property.", type(of: props[propKey]))
                continue
            }

            let escapedKey = urlEncode(propKey)
            if escapedKey.count > 255 {
                print("Visilabs: WARNING - property key cannot longer than 255 characters. When URL escaped, your key is %lu characters long (the submitted value is %@, the URL escaped value is %@). Dropping property.", UInt(escapedKey.count), propKey, escapedKey)
                continue
            }
